...
with open(os.path.join(repo_dir, "docs", "tutorial", "python", f"{tid}.html"), "w") as html_outfile:
    html_outfile.write(html_out)

with open(os.path.join(repo_dir, "docs", "tutorial", "python", f"{tid}.mdx"), "w") as mdx_outfile:
    mdx_outfile.write(f"<iframe src='{tid}.html' width='100%'></iframe>")
